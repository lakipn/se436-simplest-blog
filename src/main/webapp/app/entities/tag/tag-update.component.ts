...
import { BlogPostService } from 'app/entities/blog-post';
...
export class TagUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.blogPostService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<IBlogPost[]>) => mayBeOk.ok),
                map((response: HttpResponse<IBlogPost[]>) => response.body)
            )
            .subscribe((res: IBlogPost[]) => (this.blogposts = res), (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackBlogPostById(index: number, item: IBlogPost) {
        return item.id;
    }
